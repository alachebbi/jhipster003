"use strict";
function __export(m) {
    for (var p in m) if (!exports.hasOwnProperty(p)) exports[p] = m[p];
}
__export(require("./reclamation.model"));
__export(require("./reclamation-popup.service"));
__export(require("./reclamation.service"));
__export(require("./reclamation-dialog.component"));
__export(require("./reclamation-delete-dialog.component"));
__export(require("./reclamation-detail.component"));
__export(require("./reclamation.component"));
__export(require("./reclamation.route"));
//# sourceMappingURL=index.js.map